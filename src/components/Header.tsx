...
import { ListIcon, XIcon, ArrowLeftIcon, ArrowClockwiseIcon } from '@phosphor-icons/react';
...
export function Header({ army, isOpen, onToggle, savedLists, currentListId, onSelectList, onDeleteList, onImport }: Props) {
      ...
      <header className={styles.header}>
        <div className={styles.bar}>
          <button
            ...
            onClick={isOpen ? closeAndReset : onToggle}
                  ...
                  {listEntries.map(([id, name]) => (
                    <div key={id} className={`${styles.listRow}${id === currentListId ? ` ${styles.listRowActive}` : ''}`}>
                      <button className={styles.listName} onClick={() => { onSelectList(id); closeAndReset(); }}>
                        ...
                        {id === currentListId && <span className={styles.activeBadge}>Active</span>}
                      </button>
                      {id !== currentListId && (
