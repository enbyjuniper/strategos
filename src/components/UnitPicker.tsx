...
export function UnitPicker({
  ...
  const primaryId =
    Object.entries(attachments).find(([, ids]) => ids.includes(unitId))?.[0] ??
    null;
  const mode: "free" | "host" | "attached" = isHost
    ? "host"
    : primaryId
      ? "attached"
      : "free";
  ...
  const availableForCluster = army.units.filter(
    (u) =>
      u.id !== unitId &&
  ...
  const availableForReattach = army.units.filter(
    (u) =>
      isLeader(u) &&
      u.id !== unitId &&
      !allAttachedIds.has(u.id) &&
          ...
          <>
            ...
            <div className={styles.imageRow}>
              ...
              <div className={styles.urlRow}>
                ...
                />
                <button className={styles.urlBtn} onClick={handleUrlSubmit}>
                  Set
                </button>
              </div>
              {imageUrl && (
              ...
              {(attachments[unitId] ?? []).map((uid) => {
                ...
                return (
                  <div key={uid} className={styles.detachRow}>
                    <span className={u.isChar ? styles.nameChar : styles.name}>
                      {u.name}
                    </span>
                    <button
                      className={styles.detachBtn}
                      onClick={() => onDetachUnit(uid)}
                    >
                      Detach
                    </button>
                  </div>
        ...
        return (
          ...
          >
            ...
            <span className={styles.badges}>
              {groupSize > 0 && <Badge>+{groupSize}</Badge>}
